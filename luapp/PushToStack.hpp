...
inline void _PushValueToLuaTable(lua::NativeState hLua,lua::Table &table)
{
	...
	for ( ; ! it.isEnd() ; it++ )
	{
		...
		{
			lua::Log<<"luapp:ignore unsupported value"<<lua::log::End;
			lua::Pop(hLua, 1);           // ... [T]
			continue;
		}
		else
...
inline void PushVarToLua(lua::NativeState hLua,lua::Var &t)
{
	...
	{
		lua::Log<<"luapp:ignore unsupported data type"<<lua::log::End;
		lua_pushnil(hLua);
	}
	else
	{
		lua::Log<<"luapp:you push unknown data type"<<lua::log::End;
		lua_pushnil(hLua);
	}
}
//------------------------------------------------------------------------------
